...
def next_card():
    ...
    canvas.itemconfig(card_backgroung,image=card_front_img) 
    flip_timer=window.after(3000,func=flip_card)
    
def flip_card():
    canvas.itemconfig(card_title,text="English",fill="white")
    canvas.itemconfig(card_word,text=current_card["English"])
    canvas.itemconfig(card_backgroung,image=card_back_img)
...
card_backgroung=canvas.create_image(400,263,image=card_front_img)
